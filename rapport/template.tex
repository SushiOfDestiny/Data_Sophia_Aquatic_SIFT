...
uniforme et répétitif. Il semble raisonnable de tirer partie de la
...
est le rapport entre la norme de son gradient et celle du pixel central, qui est
...
le pourcentile de points à garder selon la moyenne de leurs courbures principales;
...
observer la variation du taux de bonnes paires trouvées par \textit{HM} en fonction de
la variation de la taille du descripteur. C'est au final une tâche d'optimisation.
Il est fort probable que cette relation soit une fonction croissante, et comme l'on
cherche non pas la taille maximisant le nombre de bons points, mais la taille caractéristique
du voisinage d'un bon point, il faudrait introduire une pénalité ou une limite
...
On pourrait même aussi incorporer la phase d'interpolation des extrema dans l'espoir
d'ajouter de la précision, voire d'en atteindre une subpixellique.

Enfin, comme présenté en introduction, plusieurs méthodes très efficaces ont été développées
ces dernières années, comme \textit{SURF} et \textit{DAISY} que nous n'avons pas analysées,
mais qui le mériteraient.

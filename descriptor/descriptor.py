...
def compute_features_overall(g_img, border_size=1):
    ...
    Resulting features are: positive eigenvalues, their associated eigenvectors,
    also negative eigenvalues and their associated eigenvectors (their opposite),
    and the gradient norms and orientations.
    """
    ...
    # compute orientations of the gradients
...
def compute_vector_histogram(
    ...
    # normalize vectors values by keypoint value
    kp_value = values[kp_position[0], kp_position[1]]
    epsilon = 1e-6
    if np.abs(kp_value) < epsilon:
        kp_value = epsilon
    normalized_values = values / kp_value

    # normalize vectors orientations with kp orientation in trigo order
    kp_orientation = vectors_orientations[kp_position] % (2 * np.pi)
    normalized_orientations = (vectors_orientations - kp_orientation) % (2 * np.pi)

    # initialize the angular histograms
    nb_angular_bins = int(2 * np.pi / delta_angle) + 1
        ...
        for bin_i in range(nb_bins):
            ...
            for j in range(y - bin_radius, y + bin_radius + 1):
                for i in range(x - bin_radius, x + bin_radius + 1):
                    ...
                    angle = normalized_orientations[j, i]
...
def display_histogram(histogram):
    """
    Display a histogram. On the abscissa is the angle, on the ordinate is the value.
    histogram: numpy array of shape (nb_angular_bins,)
    """
    nb_angular_bins = histogram.shape[0]
    x = np.linspace(0, 2 * np.pi, nb_angular_bins)
...
def compute_descriptor(overall_features, kp_position):
    """
    ...
    return descriptor: list of 3 histograms, each of shape (nb_bins, nb_bins, nb_angular_bins)
    """
    histograms = []
